...
Com a app no ar, o contrato curado é servido em `http://localhost:8080/openapi.yaml` (fonte de verdade: `src/main/resources/static/openapi.yaml`). Abra-o em qualquer visualizador OpenAPI (editor.swagger.io, Redoc). A Swagger UI está em `http://localhost:8080/swagger-ui.html` e carrega esse contrato curado (`springdoc.swagger-ui.url=/openapi.yaml`). O contrato **gera** a interface implementada pelo `QueryController` (spec-first), então o código só compila enquanto adere ao contrato.
