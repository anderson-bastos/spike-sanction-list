...
class ReusableCoreStructuralTest {
     ...
     * [com.spike.ofac.adapter.out.source.UnAdapter] / [com.spike.ofac.adapter.out.source.EuAdapter]
    ...
    private fun passthroughRawStore(folder: Path): RawSnapshotStore {
        ...
        return object : RawSnapshotStore {
            override fun put(versionId: com.spike.ofac.domain.model.VersionId, bytes: ByteArray): Path =
                fs.put(versionId, bytes)
            override fun get(versionId: com.spike.ofac.domain.model.VersionId): ByteArray =
                fs.get(versionId)
            override fun verifyIntegrity(versionId: com.spike.ofac.domain.model.VersionId): Boolean = true
            override fun delete(versionId: com.spike.ofac.domain.model.VersionId): Boolean =
                fs.delete(versionId)
        }
    }
